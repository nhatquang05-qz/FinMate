...
exports.createTransaction = async (req, res) => {
    ...
    try {
        const [categories] = await db.query('SELECT user_id FROM categories WHERE id = ?', [
            category_id,
        ]);
...
exports.updateTransaction = async (req, res) => {
    ...
    try {
        const [transactions] = await db.query('SELECT user_id FROM transactions WHERE id = ?', [
            id,
        ]);
        ...
        if (transactions[0].user_id !== userId) {
            return res
                .status(403)
                .json({ message: 'User not authorized to update this transaction' });
        }
        ...
        const sql =
            'UPDATE transactions SET amount = ?, type = ?, transaction_date = ?, note = ?, category_id = ? WHERE id = ?';
...
exports.deleteTransaction = async (req, res) => {
    ...
    try {
        ...
        if (transactions[0].user_id !== userId) {
            return res
                .status(403)
                .json({ message: 'User not authorized to delete this transaction' });
        }
...
exports.getStatistics = async (req, res) => {
    ...
    if (periodType === 'year') {
        ...
        if (!startDate || !endDate)
            return res
                .status(400)
                .json({ message: 'Start and end dates are required for weekly view.' });
    ...
    try {
        ...
        res.json({
            ...
            incomeByCategory: incomeByCategory[0].map(item => ({
                ...item,
                totalAmount: parseFloat(item.totalAmount),
            })),
        });
...
exports.getCalendarView = async (req, res) => {
    ...
    `;
        ...
        res.json({
            ...
            dailySummaries: dailySummaryResult[0].map(d => ({
                ...d,
                netAmount: parseFloat(d.netAmount),
            })),
