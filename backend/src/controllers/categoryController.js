...
exports.createCategory = async (req, res) => {
  ...
  try {
    const sql = 'INSERT INTO categories (name, type, icon, user_id, budget_limit) VALUES (?, ?, ?, ?, ?)';
    const [result] = await db.execute(sql, [name, type, icon || null, userId, budget_limit || 0]);

    res.status(201).json({ id: result.insertId, name, type, icon, user_id: userId, budget_limit });
  } catch (error) {
    console.error(error);
...
exports.updateCategory = async (req, res) => {
    ...
    try {
      ...
      if (categories[0].user_id !== userId) return res.status(403).json({ message: 'Unauthorized' });

      const sql = 'UPDATE categories SET name = ?, type = ?, icon = ?, budget_limit = ? WHERE id = ?';
