...
const askFinpetService = async (userId, message, history) => {
    ...
    try {
        ...
        const reply =
            chatCompletion.choices[0]?.message?.content || 'Xin lỗi, Finpet đang ngủ gật một chút.';
        ...
        if (error?.error?.code === 'invalid_api_key') {
            return {
                reply: 'Lỗi hệ thống: API Key của Groq bị sai hoặc hết hạn. Vui lòng kiểm tra file .env ở Backend.',
                status: 200,
            };
        }
